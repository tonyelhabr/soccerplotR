...
#'   \item{**team_abbr**}{ - The team abbreviation. Should be one of [`valid_team_names()`]. The function tries to clean team names internally by calling [`soccerplotR::clean_team_abbrs()`].}
...
#' library(ggplot2)
#'
#' team_abbr <- soccerplotR::valid_team_names()
#'
#' df <- data.frame(
#'   a = rep(1:6, 5),
...
#'   geom_eng_logos(aes(team_abbr = teams), width = 0.075) +
...
#' ggplot(df, aes(x = a, y = b)) +
#'   geom_eng_logos(aes(team_abbr = teams, alpha = alpha), width = 0.075) +
#'   geom_label(aes(label = teams), nudge_y = -0.35, alpha = 0.5) +
...
#' ggplot(df, aes(x = a, y = b)) +
#'   geom_eng_logos(aes(team_abbr = teams, alpha = alpha, colour = colour), width = 0.075) +
#'   geom_label(aes(label = teams), nudge_y = -0.35, alpha = 0.5) +
#'   scale_alpha_identity() +
...
#' ggplot(df, aes(x = a, y = b)) +
#'   geom_eng_logos(aes(team_abbr = teams), width = 0.075, alpha = 0.6) +
#'   geom_label(aes(label = teams), nudge_y = -0.35, alpha = 0.5) +
#'   theme_void()
#'
#' }
geom_eng_logos <- function(mapping = NULL, data = NULL,
...
#' @export
GeomENGlogo <- ggplot2::ggproto(
  "GeomENGlogo", ggplot2::Geom,
  ...
  draw_panel = function(data, panel_params, coord, na.rm = FALSE, country = c("ENG")) {

    country <- match.arg(country)

    team_abbr <- clean_team_abbrs(as.character(data$team_abbr), country = country, keep_non_matches = TRUE)

    data$path <- logo_from_abbr(team_abbr, country = country)
